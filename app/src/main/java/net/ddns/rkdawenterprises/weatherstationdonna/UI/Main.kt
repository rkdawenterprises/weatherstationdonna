...
{
    val are_system_bars_visible = main_view_model.are_system_bars_visible.collectAsStateWithLifecycle(initialValue = true)

    val is_dark_mode = User_settings.is_application_in_dark_mode(context).collectAsStateWithLifecycle(initialValue = false)
    ...
    val location_permission_dont_ask_again_checked_state: MutableState<Boolean> = remember { mutableStateOf(false) }
    ...
    val is_larger_window = remember { window_size_class.windowWidthSizeClass != WindowWidthSizeClass.COMPACT }
    ...
    val location_permissions_state = rememberMultiplePermissionsState(listOf(android.Manifest.permission.ACCESS_COARSE_LOCATION,
                                                                             android.Manifest.permission.ACCESS_FINE_LOCATION)) { result ->
        val is_course_permission_granted = result.getOrDefault("android.permission.ACCESS_COARSE_LOCATION",
                                                               false)
        val is_fine_permission_granted = result.getOrDefault("android.permission.ACCESS_FINE_LOCATION",
                                                             false)
        if(is_course_permission_granted || is_fine_permission_granted)
        {
    ...
    val action_menu_items: List<Action_menu_item> = remember {
        listOfNotNull(Action_menu_item(title = R.string.action_set_dart_mode_theme,
    ...
    val action_bar_items: List<Action_menu_item> = remember {
        listOfNotNull(if(is_larger_window) Action_menu_item(title = R.string.action_set_forcast_location,
                                                            title_condensed = R.string.action_set_forcast_location_condensed,
                                                            description = R.string.action_set_forcast_location,
                                                            icon = R.drawable.twotone_map_32,
                                                            on_clicked = {
                      ...
                      Action_menu_item(title = R.string.action_refresh_weather_data,
    ...
    LaunchedEffect(Unit) {
        ...
        User_settings.load_forecast_location_setting(context = context,
                                                     scope = main_view_model.viewModelScope) { location ->
            if(location.isEmpty())
            {
                action_set_forcast_location(context = context,
                                            main_view_model = main_view_model,
